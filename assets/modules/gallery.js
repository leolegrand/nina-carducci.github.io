...
export const createGallery = (gallery) => {
  ...
  gallery.map((image) => {
    ...
    DOMGallery.innerHTML += `<img srcset="${image.imageUrl}?w=320 320w, ${image.imageUrl}?w=640 640w, ${image.imageUrl}?w=1024 1024w, ${image.imageUrl}?w=1920 1920w" src="${image.imageUrl}"  alt="${image.altText}" data-tag="${image.tag}">`
  })
